...
async function getInternshipFolderId(adminid) {
...
router.post('/create', upload.array('files', 3), async (req, res) => {
  ...
  try {
    ...
    const postQuery = `
      INSERT INTO internship_posts (title, content, adminid)
      VALUES ($1, $2, $3)
      RETURNING *;
    `;
    ...
    if (req.files && req.files.length > 0) {
      const internshipFolderId = await getInternshipFolderId(adminid);

      for (const file of req.files) {
        const savedFile = await saveFileToRepository(internshipFolderId, file, adminid);
        await client.query(
          'INSERT INTO internship_post_files (post_id, file_id) VALUES ($1, $2)',
    ...
    console.error('Error creating internship post:', err);
...
router.get('/posts', async (req, res) => {
  try {
    const postsQuery = `
      ...
      LEFT JOIN internship_post_files pf ON p.id = pf.post_id
...
router.get('/trash', async (req, res) => {
  try {
    ...
    console.error('Error fetching internship trash:', err);
...
router.put('/trash/:id', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'UPDATE internship_posts SET deleted_at = NOW() WHERE id = $1 RETURNING *',
      [id]
    );
...
router.put('/restore/:id', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'UPDATE internship_posts SET deleted_at = NULL WHERE id = $1 RETURNING *',
      [id]
    );
...
router.delete('/delete/:id', async (req, res) => {
  ...
  try {
    ...
    const filesQuery = `
      ...
      JOIN internship_post_files pf ON f.id = pf.file_id
      WHERE pf.post_id = $1
    `;
    ...
    await client.query('DELETE FROM internship_post_files WHERE post_id = $1', [id]);
    await client.query('DELETE FROM internship_posts WHERE id = $1', [id]);
...
router.delete('/empty-trash', async (req, res) => {
  ...
  try {
    ...
    const trashedPosts = await client.query(
      'SELECT id FROM internship_posts WHERE deleted_at IS NOT NULL'
    );
    ...
    for (const post of trashedPosts.rows) {
      ...
      await client.query('DELETE FROM internship_post_files WHERE post_id = $1', [post.id]);
    }

    // Delete all trashed posts
    const result = await client.query('DELETE FROM internship_posts WHERE deleted_at IS NOT NULL');
...
router.put('/update/:id', upload.array('files', 3), async (req, res) => {
  ...
  try {
    ...
    await client.query('ROLLBACK');
    console.error('Error updating internship post:', err);
